...
for(uint32_t i = sx[1]; i < x[1]; i++) printf("%u ", X[1][i]);printf("\n");
    ...
    for(int t = 0; t <= 1; t++) {
        ...
        if(c[t] == 0) {
            if(x[z] == 0) {
                parameter newC = c, newR = r, newX = x;
                ...
                impPlexBranch(deep, newR, newC, newX, t);
...
printf("pivot %u, t %u, degree %u\n", maxUV[t], t, maxE[z]);fflush(stdout);
    ...
    parameter newC = plexUpdate(t, deep, r, maxUV[t], c, C);
    parameter newX = plexUpdate(t, deep, r, maxUV[t], x, X);
#ifdef DEBUG
// printf("deep4.1 %u\n", deep);fflush(stdout);
#endif
...
for(uint32_t i = 0; i < wsSize; i++) printf("%u ", ws[deep][i]);printf("\n");
    ...
    for(uint32_t i = 0; i < wsSize; i++) {
        ...
        parameter newC = plexUpdate(z, deep, r, w, c, C);
        parameter newX = plexUpdate(z, deep, r, w, x, X);

        //scan c x r
        for(uint32_t i = 0; i < newC[t]; i++)
            if(!g->connect(w, C[t][i], z)) nonNei.addNonNei(t, C[t][i], w);
        for(uint32_t i = 0; i < newX[t]; i++) 
            if(!g->connect(w, X[t][i], z)) nonNei.addNonNei(t, X[t][i], w);
        for(uint32_t i = 0; i < r[t]; i++)
            if(!g->connect(w, R[t][i], z))
                nonNei.addNonNei(t, R[t][i], w);
#ifdef DEBUG
printf("    in h node, deep %u  i %u\n", deep, i);
printf("R_l:");
for(uint32_t i = 0; i < r[0]; i++) printf("%u ", R[0][i]);printf("\n");
printf("R_r:");
for(uint32_t i = 0; i < r[1]; i++) printf("%u ", R[1][i]);printf("\n");
...
printf("    return deep4 %u\n", deep);
        ...
        for(uint32_t i = 0; i < r[t]; i++) if(!g->connect(w, R[t][i], z)) nonNei.pop(t, R[t][i]);

        X[z].changeTo(w, x[z]++);
        // R[z].changeTo(w, --r[z]);
        --r[z];
#ifdef DEBUG
printf("    out h node, deep %u  i %u\n", deep, i);
printf("R_l:");
for(uint32_t i = 0; i < r[0]; i++) printf("%u ", R[0][i]);printf("\n");
printf("R_r:");
for(uint32_t i = 0; i < r[1]; i++) printf("%u ", R[1][i]);printf("\n");
...
// #endif
    ...
    for(uint32_t i = 0; i < wsSize; i++) {
        ...
        parameter newC = plexUpdate(z, deep, r, v, c, C);
        parameter newX = plexUpdate(z, deep, r, v, x, X);
