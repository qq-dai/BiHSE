...
g->print();
    ...
    maxCandidateSets = std::max(maxCandidateSets, S[t].r - S[t].c + S[z].r - S[z].c);
        ...
        for(uint32_t u = 0; u < g->n[t]; u++) {
            ...
            #endif
            // realR[t].clear(); realR[z].clear();
            if (g->cores[t][u] < lrs[z]) continue;
            buffer[t].clear(); buffer[z].clear();

            for(uint32_t i = g->p[t][u]; i < g->p[t][u + 1]; i++) {
                uint32_t v = g->e[t][i];
                if (g->cores[z][v] < lrs[t]) continue;
                S[z].swapByPos(--S[z].c, S[z].pos(v));
            }
...
// }
            #ifdef PUTRESULTS
                printf("res:%d:%d:%d: ",totalMaximalCount, realR[0].size(), realR[1].size());
                for(uint32_t i = 0; i <= 1; i++) {
                    for(auto u: realR[i]) {
                        printf("%u ", g->old_lables[i][u]);
                    }
                    if (i == 0) printf(" : ");
                }
                printf("\n");
...
// }
                totalMaximalCount++;
                #ifdef PUTRESULTS
                    printf("res:%d:%d:%d: ",totalMaximalCount, realR[0].size()+S[0].cSize(), realR[1].size()+S[1].cSize());
                    for(uint32_t i = 0; i <= 1; i++) {
                        for(auto u: realR[i]) {
                            printf("%u ", g->old_lables[i][u]);
                        }
                        for(uint32_t j = S[i].c; j < S[i].r; j++) {
                            printf("%u ", g->old_lables[i][S[i][j]]);
                        }
